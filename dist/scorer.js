...
export function scoreTask(task, duplicateIds) {
    const breakdown = {
    ...
    if (!task.priority)
        breakdown.metadataQuality -= 5;
    if (!task.status)
        breakdown.metadataQuality -= 5;
    if (task.id && duplicateIds.has(task.id))
        breakdown.metadataQuality -= 10;
    return breakdown;
}
...
export function scoreTasks(backlog) {

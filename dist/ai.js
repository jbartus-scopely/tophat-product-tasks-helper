    ...
    return new Promise((resolve) => {
        ...
        child.stdout.on('data', (data) => {
            ...
            for (const char of text) {
                if (char === '\n') {
                    process.stdout.write(' '.repeat(Math.max(0, width - 2 - lineLen)) +
                        chalk.dim('│') + '\n' + chalk.dim('  │ '));
                    lineLen = 0;
                }
                else {
...
export async function analyzeBacklog(backlog, ask, group) {
    ...
    for (const line of lines) {
        ...
        if (inCsvBlock || (!headerSeen && /^Task\s*ID\s*,/i.test(trimmed))) {
            ...
            if (/^Task\s*ID\s*,/i.test(trimmed)) {
                headerSeen = true;
                if (!inCsvBlock)
                    inCsvBlock = false; // bare CSV, just skip header
                continue;
            }
...
export async function groomTasks(backlog, group, verbose, cache) {
...
export async function prioritizeTasks(backlog, group, verbose, cache) {
    // Focus on TODO tasks
...
export async function findDuplicates(backlog, cache) {

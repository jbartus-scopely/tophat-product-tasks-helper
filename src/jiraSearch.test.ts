...
test('executes raw JQL search with requested fields and normalizes issue output', async () => {
  ...
  if (!result.ok) assert.fail('Expected Jira search to succeed.');
...
function captureSearchRequest(input: string, init?: RequestInit): CapturedSearchRequest {
  return {
    url: input,
...
function makeIssues(startAt: number, count: number): unknown[] {

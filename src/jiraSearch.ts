...
export type JiraIssueSearchResult =
  | { ok: true; issues: JiraNormalizedIssue[]; warnings: JiraWarning[] }
...
export async function searchJiraIssues(
  ...
  if (!podFieldResult.ok) return podFieldResult;

  const fields = getSearchFields(podFieldResult.fieldId);
  const issues: JiraNormalizedIssue[] = [];
  let nextPageToken = '';
  ...
  while (issues.length < SEARCH_ISSUE_CAP) {
    ...
    const result = await jiraReadJson<JiraSearchResponsePayload>(
      {
    ...
    for (const issue of pageIssues) {
      ...
      if (issues.length >= SEARCH_ISSUE_CAP) break;
    }

    nextPageToken = readNextPageToken(result.data);
    if (shouldStopPaging(result.data, pageIssues.length, nextPageToken)) break;
  }
...
function getSearchFields(podFieldId: string | null): string[] {
  return podFieldId ? [...BASE_SEARCH_FIELDS, podFieldId] : [...BASE_SEARCH_FIELDS];
}

function shouldStopPaging(
  payload: JiraSearchResponsePayload,
  pageIssueCount: number,
  nextPageToken: string,
): boolean {
  if (pageIssueCount === 0) return true;
  if (payload.isLast === true) return true;
  return !nextPageToken;
}

function readNextPageToken(payload: JiraSearchResponsePayload): string {
...
function normalizeJiraIssue(
  ...
  params: JiraIssueSearchParams,
  baseUrl: string,
  podFieldId: string | null,
): JiraNormalizedIssue {
  const issue = isRecord(value) ? value as JiraSearchIssuePayload : {};
...
function readFixVersions(value: unknown): string[] {

...
import {
  loadJiraSavedSectionsConfig,
  parseJiraSavedSectionsConfig,
} from './jiraConfig.js';

function tempConfigPath(name = 'jira-saved-sections.json'): string {
...
test('loads saved Jira sections from config object', () => {
  ...
  if (!result.ok) assert.fail('Expected config to load.');
...
test('accepts a top-level array of saved Jira sections', () => {
  ...
  assert.equal(result.ok, true);
  if (!result.ok) assert.fail('Expected config to parse.');
...
test('returns a structured error when config file is missing', () => {
  ...
  if (result.ok) assert.fail('Expected missing config to fail.');
...
test('returns a structured error when config JSON is malformed', () => {
  ...
  assert.equal(result.ok, false);
  if (result.ok) assert.fail('Expected malformed config to fail.');
...
test('returns a structured error when required section fields are missing', () => {
  ...
  assert.equal(result.ok, false);
  if (result.ok) assert.fail('Expected invalid config to fail.');

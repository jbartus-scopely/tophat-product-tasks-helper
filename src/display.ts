...
import type { Task, BacklogData, ScoreBreakdown, GroomedTask } from './types.js';
import { MAX_SCORE } from './scorer.js';

// ── CSV File Helper ──────────────────────────────────────────

function outputCsv(content: string, filePath?: string): void {
  if (filePath) {
...
function scoreBar(score: number, width: number = 8): string {
  const ratio = Math.max(0, Math.min(1, score / MAX_SCORE));
  const filled = Math.round(ratio * width);
  const empty = width - filled;

  let color: typeof chalk;
  if (ratio >= 0.7) color = chalk.green;
  else if (ratio >= 0.45) color = chalk.yellow;
...
function groupBadge(group: string): string {
  if (!group) return chalk.dim('--');
  const groupColors: Record<string, typeof chalk> = {
...
export function printBanner(backlog: BacklogData): void {
  const totalTasks = backlog.tasks.length;
  const actionable = backlog.tasks.filter(
    (t) => !['Live', 'HOLD', ''].includes(t.status) && t.status !== ''
  ).length;
  const quickWins = backlog.tasks.filter(
...
export function printWarnings(backlog: BacklogData): void {
  if (backlog.warnings.length === 0) return;
...
export function printStats(backlog: BacklogData): void {
  printBanner(backlog);
  ...
  function miniBar(count: number, maxW: number = 20): string {
...
export function printTaskTable(tasks: Task[], title: string, options?: { showGroup?: boolean }): void {
...
function preproLabel(prepro: string): string {
  switch (prepro) {
...
export function printTaskDetail(task: Task): void {
  ...
  const breakdownFields: [string, number, number][] = [
...
function aiPriorityBadge(priority: string): string {
  const norm = priority.toLowerCase().trim();
  if (norm === 'high') return chalk.red.bold('High');
  if (norm === 'mid') return chalk.yellow.bold('Mid');
  if (norm === 'low') return chalk.green.bold('Low');
  return chalk.dim(priority || '--');
}

function aiActionBadge(action: string): string {
  const norm = action.trim().toLowerCase();
  if (norm === 'todo') return chalk.green.bold('TODO');
  if (norm === 'discard') return chalk.red.bold('Discard');
  if (norm === 'merge') return chalk.magenta.bold('Merge');
  if (norm === 'keep triage') return chalk.yellow.bold('Keep Triage');
  if (norm === 'hold') return chalk.blue.bold('Hold');
  return chalk.dim(action || '--');
}

function aiNotesFormatted(notes: string): string {
  const norm = notes.trim();
  if (!norm || norm === '--') return chalk.dim('--');
  if (/^TODO candidate/i.test(norm)) return chalk.green(norm);
  if (/^Discard.*duplicate/i.test(norm)) return chalk.red(norm);
  if (/^Discard.*too big/i.test(norm)) return chalk.magenta(norm);
  if (/^Keep in TRIAGE/i.test(norm)) return chalk.yellow(norm);
  // Legacy fallbacks
  if (/duplicate/i.test(norm)) return chalk.red(norm);
  if (/missing|incomplete|unclear|stale/i.test(norm)) return chalk.yellow(norm);
  return chalk.white(norm);
}

type GroomCategory = 'todo' | 'discard-duplicate' | 'discard-toobig' | 'keep' | 'other';

interface GroomCategoryMeta {
  title: string;
  colorFn: typeof chalk;
  icon: string;
}

const GROOM_CATEGORIES: Record<GroomCategory, GroomCategoryMeta> = {
...
const CATEGORY_ORDER: GroomCategory[] = ['todo', 'discard-duplicate', 'discard-toobig', 'keep', 'other'];

function categorizeGroomedTask(task: GroomedTask): GroomCategory {
  const action = task.aiAction.trim().toLowerCase();
  const notes = task.aiNotes.trim().toLowerCase();

  // Primary: use the AI Action field
  if (action === 'todo') return 'todo';
  if (action === 'discard') {
    if (/duplicate/i.test(notes)) return 'discard-duplicate';
    if (/too big|feature|epic/i.test(notes)) return 'discard-toobig';
    return 'discard-duplicate'; // default discard subcategory
  }
  if (action === 'merge') return 'discard-duplicate';
  if (action === 'keep triage') return 'keep';
  if (action === 'hold') return 'keep';

  // Fallback: parse from notes for backward compatibility
  if (/^todo candidate/i.test(task.aiNotes.trim())) return 'todo';
  if (/^discard.*duplicate/i.test(task.aiNotes.trim())) return 'discard-duplicate';
  if (/^discard.*too big/i.test(task.aiNotes.trim())) return 'discard-toobig';
  if (/^keep in triage/i.test(task.aiNotes.trim())) return 'keep';
  if (/duplicate/i.test(notes)) return 'discard-duplicate';
  if (/too big|feature|epic/i.test(notes)) return 'discard-toobig';
  if (/todo/i.test(notes)) return 'todo';
  return 'other';
}

function printGroomSection(tasks: GroomedTask[], meta: GroomCategoryMeta, _category: GroomCategory): void {
  console.log(
    meta.colorFn(`  [${meta.icon}] `) +
...
export function printGroomTable(tasks: GroomedTask[]): void {
  console.log('');
  ...
  const grouped = new Map<GroomCategory, GroomedTask[]>();
  for (const t of tasks) {
...
export function printGroomProse(text: string): void {
  console.log('');
  const width = 78;
  console.log(
    colors.muted('  ┌─') +
...
interface AnalyzeCategoryMeta {
  title: string;
  colorFn: typeof chalk;
  icon: string;
}

interface AnalyzeStatusGroup {
  statuses: string[];
  meta: AnalyzeCategoryMeta;
}

const ANALYZE_STATUS_GROUPS: AnalyzeStatusGroup[] = [
...
const ANALYZE_STATUS_DEFAULT_META: AnalyzeCategoryMeta = { title: 'OTHER', colorFn: chalk.dim, icon: '·' };

function analyzeActionBadge(action: string): string {
  const norm = action.trim().toLowerCase();
  if (norm === 'take next') return chalk.green.bold('Take Next');
  if (norm === 'consider') return chalk.yellow.bold('Consider');
  return chalk.dim('Skip');
}

function printAnalyzeSection(tasks: GroomedTask[], meta: AnalyzeCategoryMeta, statusMap?: Map<string, string>): void {
  console.log(
    meta.colorFn(`  [${meta.icon}] `) +
    colors.header(meta.title) +
    colors.muted(` (${tasks.length})`)
...
export function printAnalyzeTable(tasks: GroomedTask[], backlog?: BacklogData): void {
  console.log('');
  ...
  if (backlog) {
    for (const t of backlog.tasks) {
      if (t.id) statusMap.set(t.id, t.status);
    }
  }

  // Group tasks by status groups (matching the list command categories)
  const assigned = new Set<GroomedTask>();
  for (const group of ANALYZE_STATUS_GROUPS) {
    ...
    if (matching.length > 0) {
      for (const t of matching) assigned.add(t);
...
export function printAnalyzeProse(text: string): void {
  console.log('');
  const width = 78;
  console.log(
    colors.muted('  ┌─') +
...
type PrioritizeCategory = 'prioritize' | 'keep-todo';

interface PrioritizeCategoryMeta {
  title: string;
  colorFn: typeof chalk;
  icon: string;
}

const PRIORITIZE_CATEGORIES: Record<PrioritizeCategory, PrioritizeCategoryMeta> = {
  'prioritize': { title: 'PROMOTE TO PRIORITIZED',  colorFn: chalk.green, icon: '^' },
  'keep-todo':  { title: 'KEEP IN TODO',            colorFn: chalk.blue,  icon: '-' },
};

const PRIORITIZE_CATEGORY_ORDER: PrioritizeCategory[] = ['prioritize', 'keep-todo'];

function categorizePrioritizeTask(task: GroomedTask): PrioritizeCategory {
  const action = task.aiAction.trim().toLowerCase();
  if (action === 'prioritize') return 'prioritize';
  return 'keep-todo';
}

function printPrioritizeSection(tasks: GroomedTask[], meta: PrioritizeCategoryMeta): void {
  console.log(
    meta.colorFn(`  [${meta.icon}] `) +
    colors.header(meta.title) +
    colors.muted(` (${tasks.length})`)
...
function prioritizeActionBadge(action: string): string {
  const norm = action.trim().toLowerCase();
  if (norm === 'prioritize') return chalk.green.bold('Prioritize');
  return chalk.blue.bold('Keep TODO');
}

export function printPrioritizeTable(tasks: GroomedTask[]): void {
  console.log('');
  ...
  // Group tasks by category
  const grouped = new Map<PrioritizeCategory, GroomedTask[]>();
  for (const t of tasks) {
...
export function printPrioritizeProse(text: string): void {
  console.log('');
  const width = 78;
  console.log(
    colors.muted('  ┌─') +
...
export function printCsv(tasks: Task[], filePath?: string): void {
  const lines: string[] = ['Task ID,Score,AI Description,AI Priority,AI Notes'];

  for (const t of tasks) {
    const desc = truncate(t.description, 80).replace(/,/g, ';').replace(/"/g, "'");
    const pri = t.priority || '--';
    const notes: string[] = [];
    if (!t.priority) notes.push('No priority set');
    if (t.status === 'BLOCK') notes.push('Blocked');
    if (t.preproWork === '2 - High') notes.push('High prepro effort');
    if (t.risk === 'High') notes.push('High risk');
...
export function printGroomCsv(tasks: GroomedTask[], backlog?: BacklogData, filePath?: string): void {
  const lines: string[] = ['Task ID,Score,AI Description,AI Priority,AI Action,AI Notes,AI Group'];

  // Build a score lookup from the backlog if available
  const scoreMap = new Map<string, number>();
  if (backlog) {
    for (const t of backlog.tasks) {
      if (t.id) scoreMap.set(t.id, t.score);
    }
  }
...
export function printTaskDetailCsv(task: Task, filePath?: string): void {
...
export function printStatsCsv(backlog: BacklogData, filePath?: string): void {
  const lines: string[] = [];
